#############################################################
# 单细胞成对差异分析 (L vs R) 与 GO 富集绘图脚本 (v6.0 Cd1主序版)
...
run_L_vs_R_go_analysis <- function(
  ...
  p <- ggplot(plot_data, aes(x = EnrichFactor, y = Description)) +
    ...
    # 【核心：等宽设置】
    # 使用 space="fixed" 强制所有分面宽度完全一致！
    # 使用 scales="free_x" 允许每个分面的 X 轴刻度独立
    facet_grid(~ Group, scales = "free_x", space = "fixed")
  
  # 保存
  filename <- paste0("Dotplot_L_vs_R_", target_direction, "_Cd1_Sorted.pdf")
  plot_height <- max(6, length(unique(plot_data$Description)) * 0.4) 
  
  ggsave(file.path(output_dir, filename), p, width = 12, height = plot_height)
