...
# 定义要分析的细胞类型
celltypes <- c("Hepatocytes", "HSCs", "LESCs")

# 为每个细胞类型创建输出目录
dir.create("subpopulation_analysis", showWarnings = FALSE)

# 对每个细胞类型进行亚群分析
for (celltype in celltypes) {
  # 创建细胞类型特定目录
  cell_dir <- file.path("subpopulation_analysis", celltype)
  dir.create(cell_dir, showWarnings = FALSE, recursive = TRUE)
  
  cat(paste0("\nProcessing ", celltype, "...\n"))
  
  # 提取特定细胞类型 - 现在所有类型都从seurat_clusters中获取
  tryCatch({
    # 检查该细胞类型是否存在于聚类中
    ...
    if(!is.null(markers) && nrow(markers) > 0) {
      write.csv(
        markers,
        file = file.path(cell_dir, paste0(celltype, "_markers.csv")),
        quote = FALSE,
        row.names = FALSE
      )
      cat("Saved markers to", file.path(cell_dir, paste0(celltype, "_markers.csv")), "\n")
    } else {
    ...
    p1 <- DimPlot(
      ...
      ggtitle(paste("UMAP of", celltype))
    
    ggsave(
      file.path(cell_dir, paste0(celltype, "_umap.pdf")),
      p1, 
      width = 10, 
      height = 8, 
      dpi = 300
    )
    cat("Saved UMAP plot to", file.path(cell_dir, paste0(celltype, "_umap.pdf")), "\n")
    
    # 保存处理后的Seurat对象
    saveRDS(
      subset_data,
      file = file.path(cell_dir, paste0(celltype, "_processed.rds"))
    )
    cat("Saved processed Seurat object to", file.path(cell_dir, paste0(celltype, "_processed.rds")), "\n")
