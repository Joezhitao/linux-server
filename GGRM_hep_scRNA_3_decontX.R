...
pbmc <- readRDS("/home/lin/GGRM_hep/result/CRC_scRNA_harmony.rds")
scobj <- pbmc
...
## 查看去污染后的结果
p1 <- DimPlot(scobj, reduction = "tsne", group.by = "orig.ident", pt.size=0.4) + ggtitle("before decoutX")
p2 <- DimPlot(low_con_scobj, reduction = "tsne", group.by = "orig.ident", pt.size=0.4, label=T, repel=T) + ggtitle("after decoutX")

plot_grid(p1, p2, ncol = 2)

# 保存RDS文件
saveRDS(low_con_scobj, file = "/home/lin/GGRM_hep/result/GGRM_hep_decontX_11m26d.rds")
