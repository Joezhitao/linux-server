          ...
          n = 1)
{
  ...
  process_LR_pair <- function(i) {
    ...
    # 创建P.spatial的副本，避免修改原始数据
    P.spatial_i <- P.spatial
    
    if (i > nLR1) {
      P.spatial_i <- P.spatial_i * adj.contact
    }
    
    P1_Pspatial <- P1 * P.spatial_i
    
    if (sum(P1_Pspatial) == 0) {
      Pnull = P1_Pspatial
      prob_i <- Pnull
      p = 1
      pval_i <- matrix(p, nrow = numCluster, ncol = numCluster, byrow = FALSE)
    } else {
      ...
      Pboot <- sapply(X = 1:nboot, FUN = function(nE) {
        ...
        Pboot = P1.boot * P2.boot * P3.boot * P4.boot * P.spatial_i
    ...
    return(list(prob = prob_i, pval = pval_i, index = i))
  }
  
  # 使用进度条显示处理进度
  cat("Processing LR pairs in parallel:\n")
  pb <- txtProgressBar(min = 0, max = nLR, style = 3)
  
  # 使用future.apply进行并行计算
  results <- future.apply::future_lapply(1:nLR, function(i) {
    result <- process_LR_pair(i)
    # 使用一个临时文件来更新进度条，避免并行处理中的冲突
