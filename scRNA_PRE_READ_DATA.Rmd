...
knitr::opts_knit$set(root.dir = "/home/lin/gfp")
...
```{r empty drop}
# 获取目录列表 - 查找所有样本文件夹
dir_name <- list.files('/home/lin/gfp_scRNA/')
print("发现的样本目录:")
print(dir_name)
...
for(i in 1:length(dir_name)){
  ...
  counts <- Read10X(data.dir = file.path("/home/lin/gfp_scRNA/", dir_name[i]))
  ...
  if(called_cells_final < 50) {
    ...
    counts <- Read10X(data.dir = file.path("/Users/joe/Desktop/gfp", dir_name[i]))
    total_counts <- Matrix::colSums(counts)
...
```{r}
# 保存RDS文件
saveRDS(scRNAlist_merge, file = "gfp_09m28d.rds")
print("数据已保存为: gfp_07m15d.rds")
```
